...
public class PgServerBuilder
{
    ...
    public async Task<PgEnvironment> BuildAsync(Action<PgServerBuilderOptions> builder, CancellationToken cancellationToken = default)
    {
        ...
        var instanceOptions = builderOptions.InstanceOptions;
        ...
        if (buildEnvironment)
        {
            if (hasEnvironment)
            {
                await DestoryExistingAsync(builderOptions, cancellationToken).ConfigureAwait(false);
            }

            var installationSource = new PgInstallationSource(builderOptions.CacheDirectory);
            installationSource.UseMain(builderOptions.ServerArtifact);
            foreach (var ext in builderOptions.extensions)
            {
    ...
    private async Task DestoryExistingAsync(PgServerBuilderOptions builderOptions, CancellationToken cancellationToken = default)
    {
        ...
        // Create a temp serer so we can issue a stop command
